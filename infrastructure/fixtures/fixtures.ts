...
export const test = base.extend<Fixtures>({
  ...
  loginData: async ({ request, userData, stateContext }: { request: APIRequestContext; userData: UserData; stateContext: StateContext }, use: (arg0: any) => any) => {
  ...
  wheelSpinData: async ({ request, stateContext }: { request: APIRequestContext; stateContext: StateContext }, use: (arg0: any) => any) => {
    const wheelSpinData = await WheelSpin(
      request,
      testData.wheelSpin,
      stateContext.accessToken
    );
